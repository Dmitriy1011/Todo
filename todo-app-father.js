(function () {
    ...
    function createApp(container, title = "List of to do") {
        ...
        if (localStorage.getItem('fatherKeys')) {
            let newValuesArray = JSON.parse(localStorage.getItem('fatherKeys'));
                ...
                newValuesArrayItem.buttonDone.addEventListener('click', () => {
                    for (let item of valuesArray) {
                        if (item['name'] === valuesArray[i].name) {
                            item['done'] = !item['done'];
                            localStorage.setItem('fatherKeys', JSON.stringify(valuesArray));
                        }
                    }
